...
void AsyncFSWebServer::begin(FS* fs) {
    ...
    if (AP_ENABLE_BUTTON >= 0) {
        if (_apConfig.APenable || AP_ALWAYS) {
...
bool AsyncFSWebServer::load_pixel() {
    File configFile = _fs->open(PIXEL_FILE, "r");
    String param_name;
    if (!configFile) {
        DEBUGLOG("Failed to open pixel file");
        return false;
    }
    ...
    Serial.println(temp);
#endif
    setDodecaStarMode(json["dodecastar_mode"]);
    setPixelCount(json["pixel_count"]);
    for(int i = 0; i < getPixelCount(); i++)
    {
      param_name = "pixel" + String(i);
      _DodecaStarPixel[i].onoff = json[param_name][0].as<bool>();
      _DodecaStarPixel[i].color = webColorToInt(json[param_name][1]);
    }
    setAnimateCount(json["animate_count"]);
    for(int i = 0; i < getAnimateCount(); i++)
    {
      param_name = "animate" + String(i);
      _DodecaStarAnimate[i].order = json[param_name][0];
      _DodecaStarAnimate[i].effect = animEffectNameToInt(json[param_name][1]);
      _DodecaStarAnimate[i].duration = json[param_name][2];
      _DodecaStarAnimate[i].speed = json[param_name][3];
      _DodecaStarAnimate[i].density = json[param_name][4];
      _DodecaStarAnimate[i].color = webColorToInt(json[param_name][5]);
    }
...
bool AsyncFSWebServer::save_pixel() {
    //flag_config = false;
    DEBUGLOG("Save pixel\r\n");
    DynamicJsonBuffer jsonBuffer(4096);
    //StaticJsonBuffer<1024> jsonBuffer;
    JsonObject& json = jsonBuffer.createObject();
    json["dodecastar_mode"] = getDodecaStarMode();
    json["pixel_count"] = getPixelCount();
    for(int i = 0; i < getPixelCount(); i++)
    {
      JsonArray& jsonpixel = json.createNestedArray("pixel"+String(i));
      jsonpixel.add(_DodecaStarPixel[i].onoff ? 1 : 0);
      jsonpixel.add(intToWebColor(_DodecaStarPixel[i].color));
    }
    json["animate_count"] = getAnimateCount();
    for(int i = 0; i < getAnimateCount(); i++)
    {
      JsonArray& jsonanimate = json.createNestedArray("animate"+String(i));
      jsonanimate.add(_DodecaStarAnimate[i].order);
      jsonanimate.add(intToAnimEffectName(_DodecaStarAnimate[i].effect));
      jsonanimate.add(_DodecaStarAnimate[i].duration);
      jsonanimate.add(_DodecaStarAnimate[i].speed);
      jsonanimate.add(_DodecaStarAnimate[i].density);
      jsonanimate.add(intToWebColor(_DodecaStarAnimate[i].color));
    }
    File configFile = _fs->open(PIXEL_FILE, "w");
    if (!configFile) {
        DEBUGLOG("Failed to open pixel file for writing\r\n");
        configFile.close();
        return false;
    }
...
void AsyncFSWebServer::setPixelCount(uint8_t n)
{
  #if 0
  if(_pixelcount == 0 && n > 0)
  {
    _DodecaStarPixel = (strDodecaStarPixel *)malloc(sizeof(strDodecaStarPixel) * n);
    _pixelcount = n;
    return;
  }
  if(_pixelcount > 0 && n == 0)
  {
    free(_DodecaStarPixel);
    _pixelcount = n;
    return;
  }
  if(_pixelcount == 0 && n == 0)
  ...
  if(_pixelcount > 0 && n > 0)
  {
    _DodecaStarPixel = (strDodecaStarPixel *)realloc(_DodecaStarPixel, sizeof(strDodecaStarPixel) * n);
    _pixelcount = n;
    return;
  }
  #else
...
void AsyncFSWebServer::setAnimateCount(uint8_t n)
{
  _DodecaStarAnimate = (strDodecaStarAnimate *)realloc(_DodecaStarAnimate, sizeof(strDodecaStarAnimate) * n);
  _animatecount = n;
  return;
}

void AsyncFSWebServer::send_dodecastar_pixelmode_configuration_values_html(AsyncWebServerRequest *request) {
    String values = "";

    values += "setcolor|" + intToWebColor(_DodecaStarSetColor) + "|input\n";
    values += "colorapply|" + String(_DodecaStarColorApply ? "checked" : "") + "|chk\n";
    for(int i = 0; i < _pixelcount; i++)
    {
      #if 0
      values += "pixel" + String(i) + "|" + (String)(_DodecaStarPixel[i].onoff ? "●" : " ") + "|input\n";
      values += "pixel" + String(i) + "|" + intToWebColor(_DodecaStarPixel[i].color) + "|style.bgcolor\n";
      uint32_t color = _DodecaStarPixel[i].color, contrast = 0;
      if( (color & 0xFF) + ( (color >> 8) & 0xFF) + ( (color >> 16) & 0xFF) < 300)
        contrast = 0xFFFFFF;  // black/white constrast color for beter visibility
      values += "pixel" + String(i) + "|" + intToWebColor(contrast) + "|style.color\n";
      #else
      if(_DodecaStarPixel[i].onoff)
      { // pixel is ON
        values += "pixel" + String(i) + "|" + String(" ") + "|input\n";
        values += "pixel" + String(i) + "|" + intToWebColor(_DodecaStarPixel[i].color) + "|style.bgcolor\n";
        values += "pixel" + String(i) + "|#000|style.color\n";
        values += "pixel" + String(i) + "|#EEE|style.bordercolor\n";
      }
      else
      { // pixel is OFF
        values += "pixel" + String(i) + "|" + String("●") + "|input\n";
        values += "pixel" + String(i) + "|" + intToWebColor(_DodecaStarPixel[i].color) + "|style.color\n";
        values += "pixel" + String(i) + "|#000|style.bgcolor\n";
        values += "pixel" + String(i) + "|#333|style.bordercolor\n";
      }
      #endif
    }
    // method to prevent clicking on "Apply" or "Save"
    // until this state data has been transmitted to web interface
    values += "Apply|Apply|input\n";
    values += "Save|Save|input\n";
    request->send(200, "text/plain", values);
...
void AsyncFSWebServer::send_dodecastar_pixelmode_configuration_html(AsyncWebServerRequest *request) {
    DEBUGLOG("%s %d\n", __FUNCTION__, request->args());
    bool apply = false, save = false;
    if(request->args() > 0)  // Apply or Save Settings
    {
        _DodecaStarColorApply = false;
        for(int i = 0; i < request->args(); i++)
        {
          if(request->argName(i) == "setcolor")
          {
            _DodecaStarSetColor = webColorToInt(urldecode(request->arg(i)));
            continue;
          }
          if(request->argName(i) == "colorapply")
          {
            _DodecaStarColorApply = true;
            continue;
          }
          if(request->argName(i) == "Save" && request->arg(i) == "Save")
          {
            save = true;
            continue;
          }
          if(request->argName(i) == "Apply" && request->arg(i) == "Apply")
          {
            apply = true;
            continue;
          }
        ...
        for(int i = 0; i < request->args(); i++)
        {
          for(int j = 0; j < _pixelcount; j++)
            if(request->argName(i) == "pixel" + String(j)) {
                String value = urldecode(request->arg(i));
                bool onoff = value == "●" ? 1 : 0;
                bool valid = (value == "●" || value == " ");
                if( _DodecaStarPixel[j].onoff != onoff && valid)
                {
                  if(_DodecaStarColorApply)
                    _DodecaStarPixel[j].color = _DodecaStarSetColor; // apply color
                  // _DodecaStarSetColor = _DodecaStarPixel[j].color; // load color input
                  _DodecaStarPixel[j].onoff = onoff;
                }
                DEBUGLOG("pixel%d: %d\n", j, _DodecaStarPixel[j].onoff ? 1 : 0);
                continue;
            }
        }
        if(apply)
          _DodecaStarMode = DSM_pixel;
        if(save) {
          DEBUGLOG("pixel save\n");
          save_pixel();
        }
    }
    handleFileRead("/mode_pixel.html", request);
}

uint32_t AsyncFSWebServer::webColorToInt(String webcolor)
{
  char hexcolor[10];
  webcolor.substring(1).toCharArray(hexcolor, 7);
  return strtol(hexcolor, NULL, 16);
}

String AsyncFSWebServer::intToWebColor(uint32_t intcolor)
{
  char hexcolor[10];
  sprintf(hexcolor, "#%06X", intcolor);
  return String(hexcolor);
}

int AsyncFSWebServer::animEffectNameToInt(String anim)
{
  for(int ae = 0; ae < AE_n; ae++)
    if(anim == _AnimEffect_name[ae])
      return ae;
  return -1; // not found
}

String AsyncFSWebServer::intToAnimEffectName(int n)
{
  if(n >= 0 && n < AE_n)
    return _AnimEffect_name[n];
  return ""; // not found
}

void AsyncFSWebServer::send_dodecastar_animatemode_configuration_values_html(AsyncWebServerRequest *request) {
    String values = "";

    for(int i = 0; i < 3; i++)
    {
      values += "order" + String(i) + "|" + (String)(_DodecaStarAnimate[i].order) + "|input\n";
      values += "effect" + String(i) + "|" + intToAnimEffectName(_DodecaStarAnimate[i].effect) + "|select\n";
      values += "duration" + String(i) + "|" + (String)(_DodecaStarAnimate[i].duration) + "|input\n";
      values += "speed" + String(i) + "|" + (String)(_DodecaStarAnimate[i].speed) + "|input\n";
      values += "density" + String(i) + "|" + (String)(_DodecaStarAnimate[i].density) + "|input\n";
      values += "color" + String(i) + "|" + intToWebColor(_DodecaStarAnimate[i].color) + "|input\n";
    }
    // method to prevent clicking on "Apply" or "Save"
    // until this state data has been transmitted to web interface
    values += "Apply|Apply|input\n";
...
void AsyncFSWebServer::send_dodecastar_animatemode_configuration_html(AsyncWebServerRequest *request) {
    bool apply = false, save = false;
    DEBUGLOG("%s %d\n", __FUNCTION__, request->args());
    if (request->args() > 0)  // Save Settings
    {
        // first we must find valid apply or save click
        for (uint8_t i = 0; i < request->args(); i++)
        {
          if(request->argName(i) == "Save" && request->arg(i) == "Save")
          {
        ...
        for (uint8_t i = 0; i < request->args(); i++)
        {
            for(int j = 0; j < getAnimateCount(); j++)
            {
                if (request->argName(i) == "order" + String(j))
                {
                    _DodecaStarAnimate[j].order = urldecode(request->arg(i)).toInt();
                    //DEBUGLOG("order%d: %d\n", j, _DodecaStarAnimate[j].order);
                    continue;
                }
                if (request->argName(i) == "effect" + String(j))
                {
                    _DodecaStarAnimate[j].effect = animEffectNameToInt(urldecode(request->arg(i)));
                    continue;
                }
                if (request->argName(i) == "duration" + String(j))
                {
                    _DodecaStarAnimate[j].duration = urldecode(request->arg(i)).toInt();
                    //DEBUGLOG("effect%d: %d\n", j, _DodecaStarAnimate[j].order);
                    continue;
                }
                if (request->argName(i) == "speed" + String(j))
                {
                    _DodecaStarAnimate[j].speed = urldecode(request->arg(i)).toInt();
                    //DEBUGLOG("effect%d: %d\n", j, _DodecaStarAnimate[j].order);
                    continue;
                }
                if (request->argName(i) == "density" + String(j))
                {
                    _DodecaStarAnimate[j].density = urldecode(request->arg(i)).toInt();
                    //DEBUGLOG("effect%d: %d\n", j, _DodecaStarAnimate[j].order);
                    continue;
                }
                if (request->argName(i) == "color" + String(j))
                {
                    _DodecaStarAnimate[j].color = webColorToInt(urldecode(request->arg(i)));
                    //DEBUGLOG("effect%d: %d\n", j, _DodecaStarAnimate[j].order);
                    continue;
                }
            }
    ...
    handleFileRead("/mode_animate.html", request);
}
...
void AsyncFSWebServer::serverInit() {
    ...
    on("/mode_pixel", [this](AsyncWebServerRequest *request) {
        if (!this->checkAuth(request))
            return request->requestAuthentication();
        this->send_dodecastar_pixelmode_configuration_values_html(request);
    });
    on("/mode_pixel.html", [this](AsyncWebServerRequest *request) {
        if (!this->checkAuth(request))
            return request->requestAuthentication();
        this->send_dodecastar_pixelmode_configuration_html(request);
    });
    on("/mode_animate", [this](AsyncWebServerRequest *request) {
        if (!this->checkAuth(request))
            return request->requestAuthentication();
        this->send_dodecastar_animatemode_configuration_values_html(request);
    });
    on("/mode_animate.html", [this](AsyncWebServerRequest *request) {
        if (!this->checkAuth(request))
            return request->requestAuthentication();
        this->send_dodecastar_animatemode_configuration_html(request);
    });
